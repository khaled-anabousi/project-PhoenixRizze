 ...
 * Reads from IndexedDB via FeasibilityDB wrapper.
  ...
  function validateDataCompleteness(data) {
    const missing = [];
    const has = (k) => data && data[k] !== undefined && data[k] !== null && String(data[k]).trim() !== '';
    // Cover/basic info
    ...
    // Marketing core
    if (!has('marketingCost')) missing.push('Marketing');
    // Technical core
    if (!(has('propertyPrice') || has('requiredArea') || has('equipmentList'))) missing.push('Technical');
    // Financials
    if (!has('financialStatements')) missing.push('Financial');
    return missing;
  }
...
console.debug(`[buildPrompt] Prompt generated for ${effectiveSectionIds.length} sections in ${targetLang} language.`);
  ...
  async function callOpenAI(apiKey, userPrompt, extraUserStrings = []) {
    // Route ALL model calls through backend proxy only; never call OpenAI directly from frontend
    const url = CONFIG.OPENAI_API_ENDPOINT;
    // Allow direct browser-based calls to OpenAI for testing and development
    // if (!url || /(^https?:\/\/)?api\.openai\.com\//i.test(String(url))) {
    //   throw new Error("AI backend endpoint is not configured. Please contact support to enable the report generator.");
    // }
    const baseMessages = [
    ...
    const res = await fetch(url, {
      ...
      headers: Object.assign(
        ...
        (isOpenAI && CONFIG && CONFIG.OPENAI_API_KEY) ? { "Authorization": `Bearer ${CONFIG.OPENAI_API_KEY}` } : {}
      ),
    ...
    if (!res.ok) {
      ...
      throw new Error(`OpenAI request failed (status ${res.status}): ${JSON.stringify(data)}`);
    }
    ...
    const sectionInputs = {
      ...
      "1-4": include({
        ...
        marketingChannels: gen(
          ...
          Array.isArray(s.marketingChannels) ? s.marketingChannels : (Array.isArray(s.selectedChannels) ? s.selectedChannels : []),
  ...
  */
  ...
  async function generateAIReport() {
    ...
    // If an APL endpoint is configured, send FULL untrimmed payload without any truncation
    if (CONFIG.APL_ENDPOINT) {
      ...
      try {
        ...
        console.warn("APL generation failed, falling back to Chat Completions:", aplErr);
      }
    }
