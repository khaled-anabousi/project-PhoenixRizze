(() => {
    ...
    function mapKeyToSectionId(key) {
        ...
        const patterns = [
            ...
            { id: '1-18', tests: [/^financial/, /^totalcapital$/, /^operationalcosts$/, /^payback(period)?$/, /^roi(expecting|expectation)?$/, /^currency$/] },
    ...
    async function buildCompleteSimulatedSnapshot(partialUpdate) {
        ...
        const mergedAnswers = Object.assign({}, basePrimary, (partialUpdate || {}));
        const processed = buildProcessedFieldsFromAnswers(mergedAnswers);
        const sectioned = groupAnswersBySection(mergedAnswers);

        // CRITICAL: Ensure financial statements are in section 1-18
        if (mergedAnswers.financialStatements && typeof sectioned['1-18'] === 'object') {
            sectioned['1-18'].financialStatements = mergedAnswers.financialStatements;
        } else if (processed.financialStatements && typeof sectioned['1-18'] === 'object') {
            sectioned['1-18'].financialStatements = processed.financialStatements;
        }

        const finalSnapshot = Object.assign({}, mergedAnswers, processed, { 
            sections: sectioned, 
    ...
    async function saveAnswers(answers) {
        ...
        try {
            ...
            // then re-persist the FULL set to primary so no dynamic data is lost.
    ...
    function generateDistributionChannels(selectedChannelsArray) {
        const arr = Array.isArray(selectedChannelsArray) ? selectedChannelsArray : [];
        if (arr.length === 0) return '';
	...
	function generateMarketingChannels(selectedChannels, otherText = "") {
    const raw = Array.isArray(selectedChannels) ? selectedChannels : [];
    ...
    function getStrictUserData() {
		...
		return {
			...
			competitorsCount: safeParseFloat(surveyData.competitorsCount), // ✅ تصحيح
			...
			rentOption: surveyData.rentOption, // ✅ تصحيح
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const distInputs = document.querySelectorAll('input[name="distribution-channels"]');
			...
			// Marketing Data elements (unified with Pre-Commercial sector.html)
			...
			// Prefer page-native marketing channels group; fallback to legacy IDs if present
			...
			const elCompetitiveAdvantage = document.getElementById('competitive-advantage');
			// Support radio group for reachability on this page
			const reachRadios = document.querySelectorAll('input[name="customer-reach"]');
			...
			const elMarketSize = document.getElementById('market-size');
			const elPotentialCustomers = document.getElementById('customer-count');
			const elGrowthRate = document.getElementById('growth-rate');
			const elGrowthFactors = document.getElementById('growth-factors');
			const elCompetitors = document.getElementById('competitors');
			const marketGapRadios = document.querySelectorAll('input[name="market-gap"]');
			const elGapExplanation = document.getElementById('gap-explanation');
			const elGapExplanationContainer = document.getElementById('gap-explanation-container');
			const marketDemandRadios = document.querySelectorAll('input[name="market-demand"]');
			const elDemandNotes = document.getElementById('demand-notes');
		...
		if ((marketingChannelCheckboxes && marketingChannelCheckboxes.length) || legacyMarketingChannelCheckboxes.length) {
			// initialize from saved
			if (answers.marketingChannels && answers.marketingChannels.length) {
				if (marketingChannelCheckboxes && marketingChannelCheckboxes.length) {
					marketingChannelCheckboxes.forEach(cb => { if (cb) cb.checked = answers.marketingChannels.includes(cb.value); });
				}
				// legacy fallbacks
				if (cbSocialMediaAds) cbSocialMediaAds.checked = answers.marketingChannels.includes('socialMediaAds');
				if (cbContentMarketing) cbContentMarketing.checked = answers.marketingChannels.includes('contentMarketing');
				if (cbTvRadioAds) cbTvRadioAds.checked = answers.marketingChannels.includes('tvRadioAds');
				if (cbOtherChannels) cbOtherChannels.checked = answers.marketingChannels.includes('otherChannels');
			}
			updateOtherChannelsVisibility();
			if (marketingChannelCheckboxes && marketingChannelCheckboxes.length) {
				marketingChannelCheckboxes.forEach(cb => {
					cb.addEventListener('change', () => {
						answers.marketingChannels = getSelectedMarketingChannels();
						updateOtherChannelsVisibility();
						saveAnswers(answers);
					});
				});
